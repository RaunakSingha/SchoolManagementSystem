...
<?php
        ...
        $qry="select * from adminarticle";
        $res=mysql_query($qry);
        if(mysql_affected_rows()>0)
        {
            echo "<table width='100%' border='1' style='text-align:center'>";
            echo "<tr><th>Uploaded Notices</th><th>Delete</th></tr>";
            while($row=  mysql_fetch_array($res))
            {
                echo "<tr><td><a href='$row[1]'>$row[2]</a></td><td><a href='noticedelete.php?id=$row[2]'>DELETE</a></td></tr>";
            }
            echo "</table>";
        }
        else 
        {
            echo "<center><strong>No Notice Found</strong></center>";
        }
        mysql_close($h);
    ?>
<br />
<a href="adminarticle.php">Back to previous page</a>
